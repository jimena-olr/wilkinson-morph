...
class RobotImageAugmentations:
    ...
    def do_image_augmentations(self, model_df):
        ...
        save_path = f'./dl_morph_labelling/images/aug_images/'
...
class RobotImageAugmentations:
    ...
    def do_image_augmentations(self, model_df):
        ...
        os.makedirs(save_path, exist_ok=True)
        model_df = model_df[model_df['is_valid'] == 0]
        items = [(x, y) for x, y in zip(model_df['fname'], model_df['label'])]
        def worker(item):
            count = 0
            image_path, label = item[0], item[1]
            # robust filename extraction that works on Windows and any extension
            image = Path(str(image_path)).stem
            ...
            for im in rot:
                ...
                for flipped in flip:
                    file_name = os.path.join(save_path, f'aug{count}_{image}_{label}.png')
                    cv2.imwrite(file_name, flipped)
                    count += 1

        Parallel(n_jobs=os.cpu_count())(delayed(worker)(i) for i in tqdm.tqdm(items, ncols=80))
